...
#'
#' @export
install.cargo <- function(project,additional = NULL){
   message(glue::glue("you are now building rust project in {project}"))
  for (i in project){
  if (!is.null(additional)) {
...
```{r example-install.cargo}

```
  
```{r tests-install.cargo}
test_that("install.cargo works", {
  expect_true(inherits(install.cargo, "function")) 
})
```
    
# uninstall.cargo
    
```{r function-uninstall.cargo}
#' uninstall.cargo
#' 
...
#'
#' @export
uninstall.cargo <- function(project,additional = NULL){
   message(glue::glue("you are now building rust project in {project}"))
  for (i in project){
  if (!is.null(additional)) {
...
```{r example-uninstall.cargo}

```
  
```{r tests-uninstall.cargo}
test_that("uninstall.cargo works", {
  expect_true(inherits(uninstall.cargo, "function")) 
})
```
